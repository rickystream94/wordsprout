...
- [x] T035 Wire Workbox Background Sync in `frontend/vite.config.ts`: add a `workbox-background-sync` `BackgroundSyncPlugin` (or configure the Workbox `backgroundSync` option) to the `api-cache` `NetworkFirst` runtime cache entry for `/api/entries/*` PUT requests, using the sync queue tag `'WordSprout-score-sync'`; this ensures the SW `sync` event replays queued mutations on Android Chrome when connectivity is restored — the existing `online` + `visibilitychange` handlers in `main.tsx` already cover the iOS Safari fallback (FR-024, FR-025)

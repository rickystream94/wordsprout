...
.DESCRIPTION
    ...
    - Both run in APP_ENV=local mode: real OAuth authentication is required
      (Google or Microsoft), Cosmos DB is in-memory, AI enrichment returns fixture data
...
if (-not (Test-Path $settings)) {
    if (Test-Path $settingsEx) {
        ...
        Write-Host "  (APP_ENV is set to 'local' — real auth required, Cosmos DB is mocked, no Azure needed)" -ForegroundColor DarkGray
    } else {
...
# ─── Auto-populate auth settings from infra/config.json ──────────────────────

$config  = Get-Content (Join-Path $root 'infra' 'config.json') -Raw | ConvertFrom-Json
$devEnv  = $config.environments.dev

# api/local.settings.json — fill empty auth keys
$sJson   = Get-Content $settings -Raw | ConvertFrom-Json
$changed = $false

if (-not $sJson.Values.ENTRA_TENANT_ID) { $sJson.Values.ENTRA_TENANT_ID = $config.tenantId;        $changed = $true }
if (-not $sJson.Values.ENTRA_CLIENT_ID) { $sJson.Values.ENTRA_CLIENT_ID = $devEnv.entraClientId;   $changed = $true }
if (-not $sJson.Values.GOOGLE_CLIENT_ID){ $sJson.Values.GOOGLE_CLIENT_ID = $devEnv.googleClientId; $changed = $true }

if ($changed) {
    $sJson | ConvertTo-Json -Depth 5 | Set-Content $settings -Encoding UTF8
    Write-Host "✓ Populated auth settings in api/local.settings.json from infra/config.json" -ForegroundColor Yellow
}

# frontend/.env.local — always regenerated (Vite dev server reads this, not .env.dev)
$envLocal = Join-Path $frontDir '.env.local'
@"
# Auto-generated by dev.ps1 from infra/config.json — do not edit manually
VITE_ENTRA_CLIENT_ID=$($devEnv.entraClientId)
VITE_ENTRA_TENANT_ID=$($config.tenantId)
VITE_GOOGLE_CLIENT_ID=$($devEnv.googleClientId)
"@ | Set-Content $envLocal -Encoding UTF8
Write-Host "✓ Written frontend/.env.local from infra/config.json" -ForegroundColor Yellow
